...
public class NursingService {
    ...
    public List<NursingVisit> listarVisitasPorPaciente(String patientId) {
        return visitRepository.findAll().stream().filter(v -> v.getPatient().getId().equals(patientId)).toList();
    }
