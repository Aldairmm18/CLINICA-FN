...
public class CreateProcedureOrderScreen implements FunctionalityScreen {
    private final Parent root;

    public CreateProcedureOrderScreen(Role role, RoleFunctionality functionality, AppContext appContext, SceneNavigator navigator) {
        ...
        CatalogPickerPane<Procedure> picker = new CatalogPickerPane<>(appContext.getInventoryService().listProcedures());
        ...
        javafx.scene.control.Button save = new javafx.scene.control.Button("Crear orden de procedimientos");
        save.setOnAction(e -> {
            ...
            String orderNumber = "PRO-" + System.currentTimeMillis();
            ProcedureOrder order = new ProcedureOrder(orderNumber, patient, doctorPicker.getSelected(), LocalDateTime.now(), picker.getSelectedItems(), OrderStatus.CREADA, requires.isSelected(), specialist.getValue());
